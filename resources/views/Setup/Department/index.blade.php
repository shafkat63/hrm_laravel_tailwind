...
<div class="container mx-auto mt-10 px-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <h2 class="text-3xl font-extrabold text-gray-800 drop-shadow-sm mb-2 sm:mb-0">Departments Management</h2>
        <nav class="text-sm text-gray-700 mt-1" aria-label="Breadcrumb">
            <ol class="list-reset flex space-x-2">
                ...
                <li>Department Config</li>
                <li><span class="text-gray-400">/</span></li>
                <li class="font-semibold text-gray-800">Department Info</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="bg-white/90 backdrop-blur-lg shadow-3xl rounded-2xl overflow-hidden border border-gray-100">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center px-6 py-4 border-b border-gray-200">
            <h5 class="text-xl font-semibold text-gray-800 mb-2 sm:mb-0">Department Information Table</h5>
            <a href="javascript:void(0)" onclick="openModal('create')" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-4 py-2 rounded-xl transition duration-300 shadow-lg hover:shadow-xl transform hover:scale-[1.02] active:scale-[0.98]">
                <i class="bi bi-plus text-lg"></i> Add New Department
            </a>
        </div>
        ...
        <div class="overflow-x-auto p-4">
            <table id="dataTableDepartment" class="min-w-full divide-y divide-gray-200">
...
<div id="createDepartmentModal" class="fixed inset-0 hidden z-50 flex items-center justify-center p-4 overflow-y-auto">
    ...
    <div class="relative w-full max-w-lg bg-white/95 backdrop-blur-lg rounded-2xl shadow-3xl border border-white/50 p-6 mx-auto flex flex-col max-h-[90vh]">
        ...
        <div class="flex justify-between items-center mb-4 border-b border-gray-200 pb-3 flex-shrink-0">
            <h3 id="modalTitle" class="text-2xl font-bold text-gray-800">Add New Department</h3>
...
$(document).ready(function() {
    dataTable = $('#dataTableDepartment').DataTable({
        processing: true,
        serverSide: true,
        responsive: true,
        ajax: {
                ...
                render: function(data,type,row) {
                    ...
                    return `
                        <div class="flex justify-center items-center space-x-2">
                            <button class="text-blue-500 hover:text-blue-700 edit-button p-1 rounded-full hover:bg-blue-50 transition" data-id="${depId}" title="Edit Department">
                                <i class="bi bi-pencil-square text-lg"></i>
                            </button>
                            <button class="text-red-500 hover:text-red-700 delete-button p-1 rounded-full hover:bg-red-50 transition" data-id="${depId}" title="Delete Department">
    ...
    window.openModal = (mode = 'create', depId = null) => {
        currentMode = mode;
        const $modal = $('#createDepartmentModal');
        const $form = $('#createDepartmentForm');
        $form[0].reset();
        $('#departmentIdField').val('');

        if(mode === 'edit' && depId) {
            $('#modalTitle').text('Edit Department');
            $('#saveButton').text('Update Department');
            $('#departmentIdField').val(depId);

            showLoading('Loading department data...');
            $.ajax({
                url: `/Department/${depId}/edit`,
                type: 'GET',
                success: function(department) {
                    ...
                    if(department) {
                        ...
                        $('#status').val(department.status.toString());
                        $('#create_by').val(department.create_by);
                        $('#update_by').val(department.update_by);
                    }
            ...
            $('#saveButton').text('Save Department');
            $modal.removeClass('hidden');
        }
    };
    ...
    $('#createDepartmentForm').submit(function(e) {
        ...
        let url = currentMode === 'create' ? "{{ route('Department.store') }}" : `/Department/${$('#departmentIdField').val()}`;
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(res.statusCode === 200 || res.success) {
                    dataTable.ajax.reload();
                    closeModal();
                    Swal.fire('Success', res.statusMsg || 'Department saved successfully', 'success');
                } else {
    ...
    $('#dataTableDepartment tbody').on('click', '.edit-button', function() {
        const depId = $(this).data('id');
        openModal('edit', depId);
    });

    // Delete Button Click event handler
    $('#dataTableDepartment tbody').on('click', '.delete-button', function() {
        const depId = $(this).data('id');
        Swal.fire({
            title: 'Are you sure?',
    ...
    function deleteDepartment(depId) {
        $.ajax({
            ...
            beforeSend: () => showLoading('Deleting department...'),
            success: function(response) {
                Swal.close();
                let res = typeof response === 'string' ? JSON.parse(response) : response;
                if(res.statusCode === 200 || res.success) {
                    dataTable.ajax.reload();
                    Swal.fire('Deleted!', res.statusMsg || 'Department has been deleted.', 'success');
                } else {
                    Swal.fire('Error', res.statusMsg || 'Could not delete department.', 'error');
                }
            },
            error: () => Swal.fire('Error', 'Could not connect to server to delete department.', 'error')
        });
    }
});
</script>

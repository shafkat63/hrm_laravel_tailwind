...
<body class="bg-gray-50 min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
            ...
            <div class="relative">
                <button id="profileDropdownBtn"
                    ...
                    {{-- <img
                        src="{{ auth()->user()->photo ? asset('storage/' . auth()->user()->photo) : asset('assets/img/profile-img.jpg') }}"
                        --}} alt="Profile" class="w-8 h-8 rounded-full object-cover">
                    {{-- <span class="hidden md:inline text-sm font-medium text-gray-700">{{ auth()->user()->name
                        }}</span> --}}
    ...
    <!-- ======= Sidebar ======= -->
    <aside
        class="sidebar bg-white w-64 fixed top-16 left-0 h-[calc(100vh-4rem)] overflow-y-auto border-r scrollbar-hide hidden md:block">
        @include('layout.menu')
    </aside>

    <!-- ======= Main Content ======= -->
    <main class="flex-1 mt-16 md:ml-64 p-6">
        @yield('mainContent')
    </main>
